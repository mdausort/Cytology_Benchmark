...
INDEX_POSITIONS_VISION = {
    ...
    "ViT-L/14": {
        "half-up": list(range(12, 24)),  # 12 → 23
        "half-bottom": list(range(0, 12)),  # 0 → 11
        "all": list(range(24)),
    },
    "ViT-L/16": {
        "half-up": list(range(12, 24)),  # 12 → 23
    ...
    "ViT-G/14": {
        "half-bottom": list(range(0, 20)),  # 0 → 19
        "half-up": list(range(20, 40)),  # 20 → 39
        "all": list(range(40)),
    },
}
...
def apply_lora(args, clip_model):
        ...
        if args.model_name in [
            ...
            for i, layer in enumerate(bert.encoder.layer):
                if i in indices:
                    ...
                    self_attn = attn.self
                    for name, submodule in attn.self.named_children():
                        for item in args.params:
                            if item == "q" and name == "query":
                                ...
                                setattr(self_attn, name, new_attention_lora)
                                ...
                                setattr(self_attn, name, new_attention_lora)
                                list_lora_layers.append(new_attention_lora)
                    for name, submodule in attn.output.named_children():
                        if "o" in args.params and name == "dense":
                            ...
                            setattr(attn.output, name, new_attention_lora)
                            list_lora_layers.append(new_attention_lora)

        elif args.model_name in ["pubmedclip", "plip"]:
            indices = INDEX_POSITIONS_TEXT[args.position]
            text_encoder = clip_model.text_model.encoder

            for i, block in enumerate(text_encoder.layers):
                if i in indices:
                    attn = block.self_attn
                    for name, submodule in attn.named_children():
                        if name in ["q_proj", "k_proj", "v_proj", "out_proj"]:
                            if any(x in args.params for x in ["q", "k", "v", "o"]):
                                new_attention_lora = LinearLoRA(
                                    submodule,
                                    r=args.r,
                                    lora_alpha=args.alpha,
                                ...
                                setattr(attn, name, new_attention_lora)
                                list_lora_layers.append(new_attention_lora)
            ...
            raise NotImplementedError(
                "LoRA can not be implemented for text encoder because they have no text encoder."
            )
    ...
    if args.encoder in ["vision", "both"]:
        if args.model_name in [
        ...
        elif args.model_name in ["biomedclip", "conch"]:
            indices = INDEX_POSITIONS_VISION[args.backbone][args.position]
                    ...
                    for name, submodule in block.named_children():
                        if "Attention" in str(type(submodule)):
                            if any(x in args.params for x in ["q", "k", "v"]):
                                submodule.qkv = LinearLoRA(
                                    submodule.qkv,
                                    r=args.r,
                                    lora_alpha=args.alpha,
                                    dropout_rate=args.dropout_rate,
                                )
                                list_lora_layers.append(submodule.qkv)

                            if any(x in args.params for x in ["o"]):
                                submodule.proj = LinearLoRA(
                                    submodule.proj,
                                    r=args.r,
                                    lora_alpha=args.alpha,
                                    dropout_rate=args.dropout_rate,
                                )
                                list_lora_layers.append(submodule.proj)

        elif args.model_name in ["pubmedclip", "plip"]:
            indices = INDEX_POSITIONS_VISION[args.backbone][args.position]
            vision_encoder = clip_model.vision_model.encoder

            for i, block in enumerate(vision_encoder.layers):
                if i in indices:
                    attn = block.self_attn
                    for name, submodule in attn.named_children():
                        if name in ["q_proj", "k_proj", "v_proj", "out_proj"]:
            ...
            for i, block in enumerate(vision_encoder.blocks):
                if i in indices:
                    ...
                    if any(x in args.params for x in ["o", "out", "out_proj", "proj"]):
                        ...
                        list_lora_layers.append(attn.proj)

        elif args.model_name in ["vit_google-b16", "vit_google-b32"]:
            indices = INDEX_POSITIONS_VISION[args.backbone][args.position]
                    ...
                    for name, submodule in block.attention.attention.named_children():
                        for item in args.params:
                            if item == "q" and name == "query":
                                ...
                                setattr(
                                    block.attention.attention, name, new_attention_lora
                                )
                                list_lora_layers.append(new_attention_lora)
                            elif item == "k" and name == "key":
                                new_attention_lora = LinearLoRA(
            ...
            raise NotImplementedError(
                f"LoRA application not implemented for model {args.model_name}"
            )
