...
class BCFC(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if os.path.exists(os.path.join(self.splits_dir, SPLIT_FILES["train"])):
            train = self.read_txt_split(
                os.path.join(self.splits_dir, SPLIT_FILES["train"])
            )
            val = self.read_txt_split(os.path.join(self.splits_dir, SPLIT_FILES["val"]))
            test = self.read_txt_split(
                os.path.join(self.splits_dir, SPLIT_FILES["test"])
            )
        else:
    ...
    def read_txt_split(self, filepath):
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for ln, line in enumerate(f, start=1):
                ...
                if len(parts) < 2:
                    raise ValueError(
                        f"Bad line format in {filepath} at line {ln}: {line}"
                    )
