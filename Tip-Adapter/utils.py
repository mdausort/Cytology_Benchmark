...
def build_cache_model(cfg, clip_model, train_loader_cache, shot):
    ...
    if not cfg["load_cache"]:
        ...
        torch.save(
            cache_keys, cfg["cache_dir"] + "/keys_" + str(shot) + "shots.pt"
        )
        torch.save(
            cache_values, cfg["cache_dir"] + "/values_" + str(shot) + "shots.pt"
        )

    else:
        cache_keys = torch.load(
            cfg["cache_dir"] + "/keys_" + str(shot) + "shots.pt"
        )
        cache_values = torch.load(
            cfg["cache_dir"] + "/values_" + str(shot) + "shots.pt"
        )
...
def search_hp(
    cfg, cache_keys, cache_values, features, labels, clip_weights, adapter=None
):
