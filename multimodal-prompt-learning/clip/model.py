...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        ...
        return self.visual(image.to(dtype=dtype))

    def encode_text(self, text):
        dt = next(self.transformer.parameters()).dtype
        x = self.token_embedding(text).to(dtype=dt)  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.to(dtype=dt)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).to(dtype=dt)
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(li):
        if isinstance(li, (nn.Conv1d, nn.Conv2d, nn.Linear)):
            li.weight.data = li.weight.data.half()
            if li.bias is not None:
                li.bias.data = li.bias.data.half()

        if isinstance(li, nn.MultiheadAttention):
            for attr in [
                *[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]],
                ...
                tensor = getattr(li, attr)
                if tensor is not None:
                    tensor.data = tensor.data.half()

        for name in ["text_projection", "proj"]:
            if hasattr(li, name):
                attr = getattr(li, name)
                if attr is not None:
