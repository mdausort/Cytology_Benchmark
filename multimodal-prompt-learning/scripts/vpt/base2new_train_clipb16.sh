...
DIR=/gpfs/projects/acad/coalap/mdausort/ISBI_sup/output/${DATASET}/${TRAINER}/vit_l14/${SHOTS}shots/${N_CTX_VISION}nctxv_dv${PROMPT_DEPTH_VISION}/seed${SEED}
LOGFILE="${DIR}/log.txt"

echo "Task ${tid}/${total}: DATASET=${DATASET} CFG=vit_l14 SHOTS=${SHOTS} SEED=${SEED}"
echo "Output dir: ${DIR}"
...
python train.py \
  ...
  --config-file configs/trainers/${TRAINER}/vit_l14.yaml \
  --output-dir ${DIR} \
