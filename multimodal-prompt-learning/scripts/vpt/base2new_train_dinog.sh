...
PROMPT_DEPTH_TEXT=0
PROMPT_DEPTH_VISION=40
N_CTX_TEXT=4

DIR=/gpfs/projects/acad/coalap/mdausort/ISBI_sup/output/${DATASET}/${TRAINER}/dinobloom-g/${SHOTS}shots/${N_CTX_VISION}nctxv_dv${PROMPT_DEPTH_VISION}/seed${SEED}
LOGFILE="${DIR}/log.txt"

echo "Task ${tid}/${total}: DATASET=${DATASET} CFG=dinobloom-g SHOTS=${SHOTS} SEED=${SEED}"
echo "Output dir: ${DIR}"
...
python train.py \
  ...
  --config-file configs/trainers/${TRAINER}/dinobloom-g.yaml \
  --output-dir ${DIR} \
