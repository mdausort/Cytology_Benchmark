...
DIR=/gpfs/projects/acad/coalap/mdausort/ISBI_sup/output/${DATASET}/${TRAINER}/dinobloom-l/${SHOTS}shots/${N_CTX_VISION}nctxv_dv${PROMPT_DEPTH_VISION}/seed${SEED}
LOGFILE="${DIR}/log.txt"

echo "Task ${tid}/${total}: DATASET=${DATASET} CFG=dinobloom-l SHOTS=${SHOTS} SEED=${SEED}"
echo "Output dir: ${DIR}"
...
python train.py \
  ...
  --config-file configs/trainers/${TRAINER}/dinobloom-l.yaml \
  --output-dir ${DIR} \
